...
```
defroute(info):if"anthropic"in info["topic"].lower():return anthropic_chainelif"langchain"in info["topic"].lower():return langchain_chainelse:return general_chain
```
...
**API Reference:**cosine_similarity | StrOutputParser | PromptTemplate | RunnableLambda | RunnablePassthrough | OpenAIEmbeddings
```
