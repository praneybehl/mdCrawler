...
The properties of a component object are usually referred to as _options_. This is where the Options API gets its name.
...
## composable ​
The term _composable_ describes a common usage pattern in Vue. It isn't a separate feature of Vue, it's just a way of using the framework's Composition API.
...
## custom element ​
A _custom element_ is a feature of the Web Components standard, which is implemented in modern web browsers. It refers to the ability to use a custom HTML element in your HTML markup to include a Web Component at that point in the page.
...
## fragment ​
The term _fragment_ refers to a special type of VNode that is used as a parent for other VNodes, but which doesn't render any elements itself.
...
A _functional component_ is an alternative form of component that is declared using a function instead. That function acts as the render function for the component.
...
In a Vue context, reactivity is used to describe a collection of features. Those features combine to form a _reactivity system_ , which is exposed via the Reactivity API.
...
## Reactivity API ​
The _Reactivity API_ is a collection of core Vue functions related to reactivity. These can be used independently of components. It includes functions such as `ref()`, `reactive()`, `computed()`, `watch()` and `watchEffect()`.
...
## scheduler ​
The _scheduler_ is the part of Vue's internals that controls the timing of when reactive effects are run.
...
## scoped slot ​
The term _scoped slot_ is used to refer to a slot that receives props.
...
## VNode ​
A _VNode_ is a _virtual DOM node_. They can be created using the `h()` function.
