...
![LLMCompiler Graph](https://langchain-ai.github.io/langgraph/tutorials/llm-compiler/LLMCompiler/)
...
frommath_toolsimport get_math_tool
...
fromlangchain_core.runnablesimport RunnableBranch
fromlangchain_core.toolsimport BaseTool
fromlangchain_openaiimport ChatOpenAI
fromoutput_parserimport LLMCompilerPlanParser, Task
...
API Reference: BaseChatModel | ChatPromptTemplate | RunnableBranch | BaseTool | ChatOpenAI
```
...
![diagram](https://langchain-ai.github.io/langgraph/tutorials/llm-compiler/LLMCompiler/)
```
importre
importtime
fromconcurrent.futuresimport ThreadPoolExecutor, wait
fromtypingimport Any, Dict, Iterable, List, Union
...
def_get_observations(messages: List[BaseMessage]) -> Dict[int, Any]:
...
classSchedulerInput(TypedDict):
  messages: List[BaseMessage]
  tasks: Iterable[Task]


def_execute_task(task, observations, config):
...
def_resolve_arg(arg: Union[str, Any], observations: Dict[int, Any]):
  # $1 or ${1} -> 1
  ID_PATTERN = r"\$\{?(\d+)\}?"
...
```
def_parse_joiner_output(decision: JoinOutputs) -> List[BaseMessage]:
...
defselect_recent_messages(state) -> dict:
  messages = state["messages"]
  selected = []
